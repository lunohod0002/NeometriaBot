...
from app.keyboards.start.text import insurance_property_button_info,insurance_live_and_property_button_info
import os
...
@bot.callback_query_handler(func=lambda call: insurance_live_and_property_button_info.filter(call.data))
def incurance_property(call: CallbackQuery):
    incurance_live_and_property_storage.set_state(chat_id=call.message.chat.id, user_id=call.from_user.id, state="incurance_propertyd")

    incurance_live_and_property_storage.set_state(chat_id=call.message.chat.id, user_id=call.from_user.id, state="incurance_property")
    ...
    def set_property_incurance_property_bank(message: Message):
        print(message)
        incurance_live_and_property_storage.set_data(chat_id=message.chat.id, user_id=message.from_user.id,
...
def set_property_incurance_property_birthdate(message: Message):
    ...
    incurance_live_and_property_storage.set_state(message.chat.id, message.from_user.id,state="incurance_property")
    ...
    def choose_incurance_property_sex(call: CallbackQuery):
        print(call)
        incurance_live_and_property_storage.set_data(chat_id=call.message.chat.id, user_id=call.from_user.id, key="sex",
                                        value=call.data)
        bot.answer_callback_query(call.id)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
        ...
        def set_property_incurance_property_policy(message: Message):
            if message.content_type != "document":
                ...
                bot.send_message(chat_id=admin_id, text="Предварительный рассчёт\n"
                                                        ...
                                                        f"<b>Банк</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['bank']}\n"
                                                        f"<b>Сумма кредита</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['credit_balance']}\n"
                                                        f"<b>Дата рождения</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['birthdate']}\n"
                                                        f"<b>Пол</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['sex']}\n")
                mas = pdf.download_document(message.chat.id, "preliminary_calculation")
                if mas != None:
                    for i in mas:
                        bot.send_document(admin_id, open(i, 'rb'))
                        os.remove(i)
                incurance_live_and_property_storage.set_state(chat_id=message.chat.id, user_id=message.from_user.id,
    ...
    def get_text_preliminary_calculation(call: CallbackQuery):
        incurance_live_and_property_storage.set_state(call.message.chat.id,
                                         call.from_user.id, state="incurance_propertys")
        tg_id = call.data.split("[]")[1]
        incurance_live_and_property_storage.set_state(tg_id, tg_id, state="preliminary_calculation_policy")
    ...
    def get_pdf_preliminary_calculation(call: CallbackQuery):
        incurance_live_and_property_storage.set_state(call.message.chat.id, call.from_user.id,
    ...
    @bot.callback_query_handler(
        func=lambda call: call.data == "reject" and incurance_live_and_property_storage.get_state(chat_id=call.message.chat.id,
    ...
    def set_property_reject(call: CallbackQuery):
        ...
        incurance_live_and_property_storage.set_state(chat_id=call.message.chat.id, user_id=call.from_user.id,
    ...
    def property_load_policy(message: Message):
        if message.content_type != "document":
            ...
            file_info = bot.get_file(message.document.file_id)
            downloaded_file = bot.download_file(file_info.file_path)
            print(incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id))
            with open(message.document.file_name, 'wb') as f:
                f.write(downloaded_file)
            ...
            pdf.upload_document(telegram_id=message.chat.id, file_name=message.document.file_name,
                                key=message.document.file_name,
                                catalog="policy")
            bot.send_message(chat_id=message.chat.id, text='Супер! Теперь введите ваш рост')
    ...
    def set_profesion(message: Message):
        ...
        bot.send_message(chat_id=admin_id, text="Оформление полиса\n"
                                                ...
                                                f"<b>Дата рождения</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['birthdate']}\n"
                                                f"<b>Пол</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['sex']}\n"
                                                f"<b>Дата начала действия полиса</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['policy_start_date']}\n"
                                                f"<b>Процентная ставка по кредиту</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['interest_rate']}\n"
                                                f"<b>Номер кредитного договора</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['contract_number']}\n"
                                                f"<b>Дата подписания кредитного договора</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['contract_sign_date']}\n"
                                                f"<b>Дата окончания кредитного договора</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['contract_end_date']}\n"
                                                f"<b>ФИО заемщика</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['contract_initials']}\n"
                                                f"<b>Адрес объекта страхования </b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['address_insurance']}\n"
                                                f"<b>Площадь квартиры</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['address_area']}\n"
                                                f"<b>Этаж</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['floor']}\n"
                                                f"<b>Количество этажей в подъезде </b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['floor_number']}\n"
                                                f"<b>Год постройки дома</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['year_construction']}\n"
                                                 f"<b>Рост</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['height']}\n"
                                                 f"<b>Вес</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['weight']}\n"
                                                 f"<b>Верхнее и нижнее давление</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['upper_and_lower_pressure']}\n"
                                                 f"<b>Адрес электронной почты</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['email']}\n"
                                                 f"<b>Номер телефона</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['phone_number']}\n"
                                                 f"<b>Профессия</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['workplace']}\n"
                                                 f"<b>Место работы</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['position']}\n"
                                                 f"<b>Должность</b>: {incurance_live_and_property_storage.get_data(chat_id=message.chat.id, user_id=message.from_user.id)['profession']}\n",
